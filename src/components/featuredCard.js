...
const FeaturedCard = ({book}) => {
 ...
 return(
				...
				<div className="relative  overflow-hidden flex-none w-52  sm:w-1/4 lg:w-1/6" onMouseLeave={() => setShowHover(false)} onMouseEnter={() => setShowHover(true)}>
					<Link  to= { `books/${title.split(" ").join("-")}`}
					state= {{book}}  
					>
						<img src={book.image_url} alt={book.title} className='h-full w-full' />
					
		  		{showHover && <div className=" absolute inset-0 truncate  w-full h-full bg-black bg-opacity-60 text-white px-3 py-5 md:p-5">
						<p className='text-lemon font-bold mb-2 sm:mb-3 '>Available</p>
						<p className=" truncate font-bold ">{book.title}</p>
						<p className='mb-1 sm:mb-5 text-sm truncate '>{book.authors.map((author, i) => {
							return (
								<span className="" key={i}>{author.name}</span>
								);
							})}
						</p>

						<dt className='font-bold '>Genre</dt>
						<dd className='truncate mb-2  text-sm'>{book.genres.map((genre, i) => {
							return (
								<span className="" key={i}>{genre.name}{i !== book.authors.length - 1 ? ', ': ''}</span>
								);
							})}
						</dd>
						<dt className="font-bold">Tags</dt>
						<dd className='truncate mb-3 sm:mb-5'>
							{book.tags.slice(0,2).map((tag, i) => {
								return (
									<span className="text-sm" key={i}>{tag.name}{ i !== book.tags.length - 1 ? ', ': ''}</span>
								)
							})}
						</dd>
						<div className="flex items-center w-full px-">
							<div className=" mb-2 flex items-center border-r border-gray-200 ">
								<div className="flex  ">
									<div className="mr-5">
										<span>
							...
							<div className="px-3 py-2">
								...
								<dd>
									<ReactStars count={Math.floor(book.rating)} isHalf={true} size={16} color="bg-lemon" />
								</dd>
							</div>
